...
test_that("tokenize(test) returns 1xn matrix", {
  write.table(
    c('Now is the time to head west toward wilder lands'),
...
test_that("tokenize(random_file) is a matrix with 1 row and at least 1 column",
          {
            expect_that(nrow(tokenize(
              paste0('nyt-collection-text/art/', art_file)
            )),
            equals(1))
            expect_gt(ncol(tokenize(
              paste0('nyt-collection-text/art/', art_file)
            )),
            1)
            expect_that(class(tokenize(
              paste0('nyt-collection-text/art/', art_file)
            )),
            equals('matrix'))
            expect_that(nrow(tokenize(
              paste0('nyt-collection-text/music/', music_file)
            )),
            equals(1))
            expect_gt(ncol(tokenize(
              paste0('nyt-collection-text/music/', music_file)
...
test_that("bounding_box(random_word) scales correctly", {
  for (loop in 1:100) {
    ...
    expect_equal(bounding_box(rand_word, 1, vscale = 2)$height,
                 2 * strheight(rand_word))
    expect_equal(bounding_box(rand_word, 1, hscale = 3)$width,
                 3 * strwidth(rand_word))
  }
})
...
test_that("spiral_points(rand_x, rand_y) performs as expected", {
  for (loop in 1:100) {
    ...
    dist_from_start = sqrt((spirals[, 1] - rand_x) ^ 2 + (spirals[, 2] - rand_y) ^
                             2)
